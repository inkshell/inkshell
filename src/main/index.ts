...
// terminal session carries no LANG/LC_ALL/LC_CTYPE at all. With no locale set,
// macOS falls back to MacRoman for any legacy 8-bit text conversion —
// including clipboard reads done in this process and locale-aware calls in
// every `claude`/shell child spawned off `process.env` (see `pty-manager.ts`)
// — so pasting accented text (e.g. "ç") renders as mojibake ("√ß"). This is a
// macOS-specific fallback, and only kicks in when none of the three locale
// variables are set at all — a value the user (or a real terminal launch)
// already put in any of them, UTF-8 or not, is left alone.
if (
