# Contributing to InkShell

First off — thank you! InkShell is built by its community, and every issue, idea,
...
The app has a dev-only capture hook. Set `INKSHELL_SCREENSHOT` to a target path
and it writes a PNG of the window (via `capturePage`, so no screen-recording
permission is needed) then quits:

```bash
npm run build
INKSHELL_SCREENSHOT=/tmp/inkshell.png ./node_modules/.bin/electron .
# optional: INKSHELL_SCREENSHOT_DELAY=6000 to wait longer before capturing
```
...
InkShell stays a **thin, honest shell** around Claude Code:
...
2. **Read, don't write, Claude's data.** History and context come from
   `~/.claude/`, which InkShell only ever reads.
